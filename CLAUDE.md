...
- **A requested season leaves the row rather than sitting there labelled "Requested"**: the row offers seasons to get, so one already on its way is nothing to offer. The client removes the card on a successful request (waiting for the next render would make the button look like it did nothing) and `GetContentItems` filters it out afterwards. The card is found with `closest('.card')` from the button rather than passed in, so the same path works on a card Modular Home drew; only the plugin's own row is hidden when it empties, since Modular Home owns the layout of its sections
- **Requested state is in-memory** (`NextSeasonsWidgetService`), matching the watchlist rule and for the same reason: the durable answer to "do I have this season" is the library, re-checked by the next sync. Persisting it would hide a season for good after a download that never happened. It expires after `RequestedRetention` (14 days) for the same reason, now that the marker hides the card outright instead of only labelling it
