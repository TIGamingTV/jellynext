...
- **New-release filter** (opt-in, per user: `NextSeasonsRecentOnly` + `NextSeasonsRecentDays`, default off/90 days): without it the library answers "what haven't I finished" - the next season of a show that ended a decade ago ranks equal to one that premiered last week. With it, a season qualifies if its *most recent release* falls inside the window. A season with no `FirstAired` is excluded - the filter is meant to exclude by default. Filtering happens at read time in `NextSeasonsProvider`, never in `ShowsCacheService`, since the season cache is global and the setting is per user
...
- **Airing is not an unconditional pass** (`SeasonReleaseHelper.IsRecentlyReleased`): it used to be, and a weekly season is part-way through airing for its whole run, so it stayed "new" indefinitely and no window - however short - could age out a show the user had decided to skip. That defeated the setting the user reaches for. Instead, a season still airing (`AiredEpisodes < EpisodeCount` on a show that has not ended) is measured from its latest episode rather than its premiere, which keeps long and split-cour seasons in scope while they run - the reason the airing clause existed - without making them permanent. Trakt reports no last-aired date for a season, so the episode is placed from `FirstAired + (AiredEpisodes - 1) * 7 days`; an estimate landing in the future means the release was faster than weekly (a batch drop with later parts listed), the cadence cannot be trusted, and the premiere is used instead - taking the estimate anyway would put every batch-dropped season permanently inside the window
...
- **"New" is a release, not a discovery**: a next season appears whenever the user's watch progress moves, so finishing a show that ended a decade ago produces one. Only seasons that pass `SeasonReleaseHelper.IsRecentlyReleased` (latest release inside `NewSeasonNotificationWindowDays`, default 30) are announced. The window is global and separate from the per-user `NextSeasonsRecentDays` library filter: a wide library and narrow announcements is a coherent combination
...
- **`WidgetController` is the only per-user controller**: the caller comes from `IAuthorizationContext`, never from a route parameter, so one user cannot read or request from another's content. `Poster/{traktId}/{seasonNumber}` is deliberately anonymous - browsers do not attach Jellyfin's token to `img` requests - and only ever answers with public artwork
- **The plugin owns card geometry, Jellyfin owns card appearance**: the markup is still Jellyfin's (`card`/`cardBox`/`cardScalable`/`cardPadder`/`cardImageContainer`/`cardText`) so the row inherits the client's theme, but width and aspect ratio are set by the plugin's own media queries. Inheriting size from the shape classes (`overflowPortraitCard` and friends) fails where a client does not apply them: the card collapses to the width of its title, and the image container - sized as a percentage of that width - vanishes, leaving a column of bare titles. That was the desktop app's symptom. Every geometry rule is scoped under `.jellynextCard` so two classes beat the client's one, and the stylesheet is appended to `<head>` after the client's. The old `hasNativeCardStyles()` probe and `.jellynextFallback` sheet are gone: there is no longer a mode to detect
- **Cards are portrait**: what is being offered is a season, and a season's picture is a poster wherever Jellyfin and the metadata providers hold one. This also drives the image preference order - Primary, then Thumb, then Backdrop. The previous order put a *series thumbnail* first, which is frequently a scene still and made every card read as a random episode
...
- **Artwork resolution is season first, then show** (`ResolveImageAsync`): the library's season item, then the season from Jellyfin's metadata providers, then the library's show, then the show from the providers, then Trakt. The season is worth two attempts before the show because it is what the card is actually offering. `LocalLibraryService.FindSeason` deliberately *includes* virtual seasons - the opposite of `GetLocalSeasons` - because with "display missing episodes" on, Jellyfin already holds the provider's season poster for a season the user has not downloaded, which is free and exactly right. Shows are matched with `FindSeriesByAnyProviderId`; TVDB alone misses shows Jellyfin identified through TMDB, which then look like a plugin that cannot find artwork
- **`IProviderManager` is asked with the real library item when there is one**, so the providers see the server's metadata language and library options; otherwise a detached `Series` carrying the show's provider ids stands in, which is all `GetAvailableRemoteImages` needs - image providers key off ids, not library membership. `GetRemoteSearchResults` is the second attempt if that is ever rejected. The *season* probe is different: season image providers reach the parent through `Season.Series`, which only resolves for a series that is in the library, so the season lookup is skipped entirely for a show that is not - the show-level chain covers that case
- **Image resolution is lazy, behind `Poster/{traktId}/{seasonNumber}`**, never while building the list: twelve cards would otherwise mean twelve metadata lookups before anything renders. `_imageLookups` carries what the anonymous image request cannot know (ids, title, library item) from the list build to the endpoint. Trakt's `extended=images` is the last resort; hits are cached 7 days and misses 12 hours, keyed per season, so a show without artwork is not looked up on every render
- **Library artwork is redirected to, external artwork is served**: `LocalRedirect("~/Items/...")` keeps library images inside Jellyfin's own resizing and caching, and the `~/` survives a base path. Anything outside Jellyfin is fetched and returned as bytes, because redirecting the browser there fails invisibly whenever it cannot reach the host - a reverse proxy sending `img-src 'self'`, an ad blocker, filtered DNS - leaving a blank card, a successful server-side lookup and nothing in the log. Only the width is constrained on library images (`fillWidth`, no `fillHeight`): asking for both makes Jellyfin crop to that exact box, which slices a 16:9 backdrop inside a portrait card
- **`Widget/Diagnostics/{userId}`** (admin only) exists because this failure mode is close to invisible: when no image path is produced, no request is made, so there is nothing to find in the log. It reports the show and season library matches, the images each holds, both card paths, and which step of the chain produced the picture
- **The client owns the artwork fallback chain**: `imagePath` then `fallbackImagePath` then a named placeholder tile, because whether an image actually loads is only knowable in the browser. `fallbackImagePath` is the library's own image path, so a card is never blank while the show sits in the library with artwork on it. Landscape artwork is switched to `object-fit: contain` on load rather than being cropped into a strip by a portrait card
...
- **Why it exists**: the Modular Home plugin (`IAmParadox27/jellyfin-plugin-home-sections`, published as "Home Screen Sections") *replaces* the home screen. It reuses the `.homeSectionsContainer` class and orders its own sections with inline `style="order:N"` — the injected widget row has no `order`, so it defaults to `0` and lands near the top whatever the user's Top/Bottom setting says. Registering a real section is the only way to give its users placement
- **Reflection, never a package reference**: `PluginInterface.RegisterSection(JObject)` is a static method reached exactly like `TraktPluginBridge` reaches the official Trakt plugin, and for the same reason — separate `AssemblyLoadContext`s. A `PackageReference` is doubly wrong: the published NuGet package is far behind the shipping plugin, *and* adding any package pulls in `CopyLocalLockFileAssemblies`, which copies the `MediaBrowser.*` assemblies into the plugin output that must resolve from the host (see "Shared Trakt Connection")
- **The payload must be *their* `JObject`**: `ModularHomeBridge.BuildPayload` serializes to a string with `System.Text.Json`, then calls `JObject.Parse` off the *registration method's own parameter type*. Constructing a `JObject` from a Newtonsoft JellyNext shipped would be a different type across load contexts and the invoke would throw — and this way JellyNext needs no JSON dependency at all
- **The handler is resolved by string, so its names are load-bearing**: `resultsAssembly`/`resultsClass`/`resultsMethod` name `ModularHomeSectionHandler.GetResults`. Modular Home builds the class with `ActivatorUtilities.CreateInstance` against **Jellyfin's root provider** (`HomeScreenSectionsPlugin.ServiceProvider` is the injected root), which is where `PluginServiceRegistrator` registers everything, so constructor injection works. A rename compiles and fails only at runtime
- **`QueryResult<BaseItemDto>` is the contract**, and it works precisely because `MediaBrowser.Model` resolves from the default ALC in every plugin — one shared type identity, no reference needed
- **`SectionId` (`jellynext-new-seasons`) is frozen**: Modular Home uses it as its dictionary key, as the CSS class on the rendered row, *and* as the entry in every user's enabled-sections list. Changing it silently un-enables the section for everyone who had turned it on
- **Answers with real virtual-library items, not synthesized DTOs**: third-party sections are rendered by Jellyfin's stock `cardBuilder`, which derives artwork and navigation from the item id, so an id-less DTO renders as an empty card. (Modular Home's own `Discover` section does return id-less DTOs, but only because it ships a bespoke client renderer for that one section key.) Using the real item also means the card's play overlay plays the stub, which `PlaybackInterceptor` already turns into a download — the request path works with no client script at all
- **Registration is re-asserted, not done once**: Jellyfin does not order plugin startup, and Modular Home holds registrations in a plain in-memory dictionary, so a reload drops the section without telling anyone. `ModularHomeRegistrationService` retries on a timer and on `ConfigurationChanged` (the heading comes from `NextSeasonsWidgetTitle`). There is no API to *withdraw* a section, so disabling the setting makes the handler answer with nothing rather than removing the row
- **The Request button cannot come from Modular Home**: `loadSections.js` dispatches its card renderer by hardcoded section key — `Discover*` and `Upcoming*` get bespoke HTML, everything else falls through to `cardBuilder.getCardsHtml`. Nothing in the registration payload carries HTML, a template or a renderer key. So the button is added client side by `jellynext-widget.js`, which is the same `MutationObserver` technique Modular Home uses for its own request button
- **`ContentItem` → library item goes through the folder name**, built once in `VirtualLibraryManager.GetShowFolderName` and reused by `GetNextSeasonShowFolderPath`. The *folder* rather than the stub file inside it: the folder is the item carrying the show's name and poster. Duplicating the sanitization at the reading end would drift
- **Two settings, one row**: `ModularHomeIntegrationEnabled` and `ModularHomeRequestButtonEnabled` sit beside the widget's own settings and reuse `NextSeasonsWidgetTitle`/`NextSeasonsWidgetLimit`
- **Known limits, documented rather than worked around**: `PluginDefinedSection` hardcodes `ViewMode = Landscape` and ignores `limit` (portrait is reachable only via Modular Home's own per-section admin setting), and a section is opt-in per user — it stays invisible until each user ticks it in Modular Home's settings, which is the first thing to check when it "doesn't appear"
