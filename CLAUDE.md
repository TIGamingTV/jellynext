...
- **Artwork comes from the Jellyfin library first**: the show itself is normally present (the user watched an earlier season); only the *season* is missing. Wide images (Thumb, then Backdrop) before Primary, since the cards are 16:9 like the home screen's own rows. Trakt's `extended=images` is the fallback (fanart before poster for the same reason), cached in memory, with misses cached too so a show without artwork is not looked up on every render
