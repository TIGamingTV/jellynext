...
- **Artwork comes from the Jellyfin library first, then Jellyfin's metadata providers, then Trakt**: the show itself is normally present (the user watched an earlier season); only the *season* is missing. Matched with `FindSeriesByAnyProviderId` - TVDB alone misses shows Jellyfin identified through TMDB, which then fall through and look like a plugin that cannot find artwork. Backdrop before Thumb before Primary throughout: both of the first two are 16:9, but a series thumbnail is frequently a scene still that reads as a random episode. A library *poster* ranks below a provider image, since for a 16:9 card the poster is the worse picture
- **`IProviderManager` is asked with the real library item when there is one**, so the providers see the server's metadata language and library options; otherwise a detached `Series` carrying the show's provider ids stands in, which is all `GetAvailableRemoteImages` needs - image providers key off ids, not library membership. `GetRemoteSearchResults` is the second attempt if that is ever rejected. This is why the widget's pictures match the rest of the home screen: they come from the same providers
- **Image resolution is lazy, behind `Poster/{traktId}`**, never while building the list: twelve cards would otherwise mean twelve metadata lookups before anything renders. `_imageLookups` carries what the anonymous image request cannot know (ids, title, library item) from the list build to the redirect. Trakt's `extended=images` is the last resort, ordered fanart -> poster -> thumb; hits are cached 7 days and misses 12 hours, so a show without artwork is not looked up on every render
- **Artwork is served by the plugin, not redirected to**: `Poster/{traktId}` fetches the bytes and returns them. A redirect fails invisibly whenever the browser cannot reach the image host - a reverse proxy sending `img-src 'self'`, an ad blocker, filtered DNS - leaving a blank card, a successful server-side lookup and nothing in the log. Serving it makes the picture exactly as reachable as the library artwork already on the page
- **`Widget/Diagnostics/{userId}`** (admin only) exists because this failure mode is close to invisible: when no image path is produced, no request is made, so there is nothing to find in the log. It reports the library match, the images that item holds, both card paths and the resolved external URL
