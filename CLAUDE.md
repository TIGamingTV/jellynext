...
- `WidgetController.cs`: New Seasons widget contents, season requests and card artwork. The only controller that authorizes ordinary users (`[Authorize]`) and answers per caller - the user comes from `IAuthorizationContext`, never from a route parameter
